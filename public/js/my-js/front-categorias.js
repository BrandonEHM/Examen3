...
function agregarCategoria() {
  const descripcion = document.getElementById(
    "descripcionCategoriaAgregar"
  ).value;
  const observaciones = document.getElementById(
    "observacionesCategoriaAgregar"
  ).value;
    ...
    success: function (result) {
      if ((result.estado = 1)) {
        ...
        let nuevoRenglon = tabla.row
          .add([categoria.descripcion, botones])
          .node();
    ...
    success: function (result) {
      ...
      if (estado == 1) {
        ...
        categorias.forEach((categoria) => {
          ...
          let nuevoRenglon = tabla.row
            .add([categoria.descripcion, categoria.observaciones,Botones])
            .node();
          //-----Linea agregada para el ID DEL RENGLON
          $(nuevoRenglon).attr("id", "renglon_" + categoria.id);
          //-------------------------------------------------------------
  ...
  $.ajax({
    ...
    success: function (result) {
      if ((result.estado = 1)) {
        ...
        let tabla = $("#tabla-categorias").DataTable();
        tabla
          .row("#renglon_" + idEliminarCategoria)
          .remove()
          .draw();
        //---------------------------------------------------------------
...
function generarBotones(categoria) {
    ...
    Botones +=' >';
    Botones +='  <button onclick="identificaIdActualizar('+categoria.id+')" type="button" class="btn btn-primary btn-icon-xxs">';
    ...
    Botones +='>';
    Botones +='  <button onclick="identificaIdEliminar('+categoria.id+')"  type="button" class="btn btn-danger btn-icon-xxs">';
...
function actualizarCategoriaById() {
  ...
  let observacionesCategoria = document.getElementById(
    "observacionesCategoriaActualizar"
  ).value;
  $.ajax({
    method: "PUT",
    ...
    success: function (result) {
      if ((result.estado = 1)) {
        ...
        document.getElementById("nombreCategoriaActualizar").value =
          categoria.descripcion;
        document.getElementById("observacionesCategoriaActualizar").value =
          categoria.observaciones;
        //alert(categoria.descripcion)
      } else {
        alert(result.mensaje);
